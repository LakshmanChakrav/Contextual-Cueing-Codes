...
currentFolder = pwd;
dbase = strcat(currentFolder,'\Subject_', sub_name,'_',num2str(subject_number),'_Exp_OBNC.xlsx');
...
m = matfile(queueName,'Writable',true);
m.queue = q;
...
for i=9:64
    ...
    target_location_matrix (relocation_order(i)) = fix((i-2)/7);
    
    list = [1,2,3,4,5,6,7,8];
    list(fix((i-2)/7)) = [];
    distractor_location_matrix (relocation_order(i)) = list(rem((i-2),7)+1);
...
for i=65:256
    ...
    reloc_tar(relocation_order(i)) = 0;
    reloc_dis(relocation_order(i)) = 0;
    target_location_matrix (relocation_order(i)) = fix((i-41)/24);
    
    distractor = fix((i-41)/24);
    while distractor == fix((i-41)/24)
        distractor = randi(8);
    end
...
for epoch=1:3
    ...
    for i=1:256 % change to 1536
        ...
        elseif main_index(i) == 1 
            fixation_screen(wPtr,xcenter,ycenter);        
            [test_rt((epoch-1)*256 + i),test_accuracy((epoch-1)*256 + i)] = relevant_context(wPtr,imgt,q,flip_target(i),reloc_tar(i),reloc_dis(i),target_location_matrix(i),distractor_location_matrix(i));
            test_RT = cat(1,test_RT,test_rt((epoch-1)*256 + i));
            test_ACC = cat(1,test_ACC,test_accuracy((epoch-1)*256 + i));
        end
...
header = {'subject_number','training_RT','training_ACC','trial_no','test_relevance','epoch','relocation','test_RT','test_ACC'};
...
xlswrite(dbase, Reloc, 1, 'G2:G769')
xlswrite(dbase, test_RT, 1, 'H2:H769')
xlswrite(dbase, test_ACC, 1, 'I2:I769')
Screen('CloseAll');
