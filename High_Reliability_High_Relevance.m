...
currentFolder = pwd;
dbase = strcat(currentFolder,'\Exp_OBTF_Subject_', num2str(subject_number),'_',sub_name,'.xlsx');
...
for i=9:16
    ...
    fixed_config_no(relocation_order(i)) = i-8;
...
for i=17:64
    ...
    reloc_tar(relocation_order(i)) = 0;
    reloc_dis(relocation_order(i)) = 0;
    fixed_config_no(relocation_order(i)) = fix((i-11)/6);
...
%         elseif main_index(i) == 1 
            fixation_screen(wPtr,xcenter,ycenter);        
            [test_rt((epoch-1)*64 + i),test_accuracy((epoch-1)*64 + i)] = irrelevant_context(wPtr,imgt,q(fixed_config_no(i),:),flip_target(i),reloc_tar(i),reloc_dis(i),distractor_location_matrix(i));
...
header = {'subject_number','training_RT','training_ACC','trial_no','test_relevance','fixed_config_no','epoch','master_epoch','relocation','test_RT','test_ACC'};
...
xlswrite(dbase, master_epoch, 1, 'H2:H769')
xlswrite(dbase, Reloc, 1, 'I2:I769')
xlswrite(dbase, test_RT, 1, 'J2:J769')
xlswrite(dbase, test_ACC, 1, 'K2:K769')
Screen('CloseAll');
