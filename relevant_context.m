function [response_time,accuracy] = relevant_context(wPtr,imgt,q,flip_target,reloc_tar,reloc_dis,target_location,distractor_location)

    Screen('TextSize', wPtr, 22);

    monitorFlipInterval=Screen('GetFlipInterval',wPtr);
    xdim = 1366; %Check if resolution is changed on test computer
    ydim = 768;
    xcenter = xdim/2;
    ycenter = ydim/2;
