...
currentFolder = pwd;
dbase = strcat(currentFolder,'\Exp_OBTC_Subject_', num2str(subject_number),'_',sub_name,'.xlsx');
...
reloc_dis = null(100);
main_index = null(100);
target_location_matrix = null(100);
distractor_location_matrix = null(100);
relocation = null(100);

flip_target = rem(randperm(64),2);
relocation_order = randperm(64);

for i=1:8
    main_index(relocation_order(i)) = 1;
    reloc_tar(relocation_order(i)) = 1;
    reloc_dis(relocation_order(i)) = 0;
    target_location_matrix (relocation_order(i)) = i;
...
for i=9:16
    ...
    target_location_matrix (relocation_order(i)) = i-8;
    
    distractor = i-8;
    while distractor == i-8
        distractor = randi(8);
    end
...
for i=17:64
    ...
    reloc_tar(relocation_order(i)) = 0;
    reloc_dis(relocation_order(i)) = 0;
    target_location_matrix (relocation_order(i)) = fix((i-11)/6);
    
    distractor = fix((i-11)/6);
    while distractor == fix((i-11)/6)
        distractor = randi(8);
    end
...
%         if main_index(i) == 0
%             fixation_screen(wPtr,xcenter,ycenter);               
%             [test_rt(i),test_accuracy(i),total_score] = new_context(wPtr,flip_target(i),previous_score);
%             test_RT = cat(1,test_RT,test_rt(i));
%             test_ACC = cat(1,test_ACC,test_accuracy(i));
%         elseif main_index(i) == 1 
            fixation_screen(wPtr,xcenter,ycenter);        
            [test_rt((epoch-1)*64 + i),test_accuracy((epoch-1)*64 + i)] = relevant_context(wPtr,imgt,q,flip_target(i),reloc_tar(i),reloc_dis(i),target_location_matrix(i),distractor_location_matrix(i));
            test_RT = cat(1,test_RT,test_rt((epoch-1)*64 + i));
            test_ACC = cat(1,test_ACC,test_accuracy((epoch-1)*64 + i));
%         end
...
%        previous_score = total_score;
        ...
        master_epoch = cat(1,master_epoch,fix((epoch+3)/4));
        trial_no = cat(1,trial_no,(epoch-1)*64 + i);
        
        if rem(((epoch-1)*64 + i),50) == 0  
...
%             end
            ...
            avgRT = mean(test_rt((((epoch-1)*64 + i)-49):((epoch-1)*64 + i)));
            avgACC = mean(test_accuracy((((epoch-1)*64 + i)-49):((epoch-1)*64 + i)))*100;
...
header = {'subject_number','training_RT','training_ACC','trial_no','test_relevance','epoch','master_epoch','relocation','test_RT','test_ACC'};
...
xlswrite(dbase, epoch_log, 1, 'F2:F769')
xlswrite(dbase, master_epoch, 1, 'G2:G769')
